...
Create an API specification (API contract) using the Swagger Editor, generate code based on a specification using Swagger Codegen, and visualize the specification in an easy-to-read way using Swagger UI.

We will be downloading and using these tools locally to develop an API for the backend of an application that manages store orders. Our API will retrieve existing orders, create new orders, change an order’s status, and delete existing orders.
...
1. Download or clone the code from: <https://github.com/swagger-api/swagger-editor>  
...
The Swagger Editor should now be open in a browser window.
...
Make sure the components section is not nested at all.  
